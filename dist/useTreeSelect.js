...
const asyncOrAsyncBlock = (it
// eslint-disable-next-line @typescript-eslint/no-explicit-any
) => {
    return (function getReturn(result
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ) {
        if (result.done) {
            return result.value;
...
const getPathToNode = (toNode, getParent) => {
    function* it() {
        var _a, _b;
        const path = [];
        let parent = (_a = (yield toNode instanceof FreeSoloNode
            ? toNode.parent
            : getParent(toNode))) !== null && _a !== void 0 ? _a : null;
        while (parent !== null) {
            path.push(parent);
            parent =
...
export const useTreeSelect = ({ branch: branchProp, branchDelimiter = " > ", componentName = "useTreeSelect", defaultBranch, defaultValue, filterOptions: filterOptionsProp = defaultFilterOptions, freeSolo, getPathLabel: getPathLabelProp, getChildren, getOptionDisabled: getOptionDisabledProp = defaultGetOptionDisabled, getOptionLabel: getOptionLabelProp = defaultGetOptionLabel, getParent, groupBy: groupByProp, inputValue: inputValueProp, isBranch: isBranchProp, isBranchSelectable = defaultIsBranchSelectable, isOptionEqualToValue: isOptionEqualToValueProp, multiple, onBranchChange, onChange: onChangeProp, onClose: onCloseProp, onHighlightChange: onHighlightChangeProp, onInputChange: onInputChangeProp, onOpen: onOpenProp, open: openProp, value: valueProp, }) => {
    ...
    const [curBranch, setBranch] = useControlled({
        ...
        default: defaultBranch !== null && defaultBranch !== void 0 ? defaultBranch : null,
        name: componentName,
        state: "branch",
    });
    ...
    const isBranch = useCallback((node) => (isBranchProp ||
        ((node) => {
            ...
            return !!result;
        }))(node), [getChildren, isBranchProp]);
    const pathArg = useMemo(() => {
        if ((curBranch !== null && curBranch !== void 0 ? curBranch : null) === null) {
            return [];
        }
    ...
    const optionsResult = usePromise(useMemo(() => {
        const options = [];
        function* getOpts() {
            ...
            if (curBranch !== null && curBranch !== void 0 ? curBranch : null !== null) {
                options.push(new InternalOption(curBranch, NodeType.UP_BRANCH, path.slice(1)));
            }
            options.push(...(yield (() => {
                const options = [];
                function* parseChildNode(childNode) {
                    if ((yield isBranch(childNode))) {
                        options.push(new InternalOption(childNode, NodeType.DOWN_BRANCH, path));
    ...
    const valueResult = usePromise(useMemo(() => {
        if ((curValue !== null && curValue !== void 0 ? curValue : null) === null) {
            return null;
        }
        else if (multiple) {
            const multiValue = [];
            ...
            for (const node of curValue) {
                ...
                if (path instanceof Promise) {
                    hasPromise = true;
                    multiValue.push(path.then((path) => new InternalOption(node, NodeType.LEAF, path)));
                }
                else {
            ...
            return path instanceof Promise
                ? path.then((path) => new InternalOption(curValue, NodeType.LEAF, path))
    ...
    const value = useMemo(() => {
        ...
        if (multiple) {
            ...
            return ((_a = valueResult.data) !== null && _a !== void 0 ? _a : ((curValue !== null && curValue !== void 0 ? curValue : null) === null
                ? null
    ...
    const isOptionEqualToValue = useCallback((option, value) => {
         ...
         */
        const optionNode = multiple && freeSolo && typeof option === "string"
        ...
        if (isOptionEqualToValueProp) {
            ...
            if (value.node instanceof FreeSoloNode) {
                return (value.node.toString() === optionNode.toString() &&
    ...
    const getOptionLabel = useCallback((arg) => getOptionLabelProp(arg.node), [getOptionLabelProp]);
            ...
            if (freeSolo &&
                ...
                (multiple ||
                    ...
                    getOptionLabel(value) !==
                        inputValue)) {
                const freeSoloOption = new InternalOption(new FreeSoloNode(inputValue, curBranch), NodeType.LEAF, pathResult.data || []);
                if ((multiple ? value : [value]).every((value) => 
                // NOT the following
                !(value &&
    ...
    const handleGroupBy = useCallback(({ node, type }) => {
    ...
    const noOptions = useRef(!options.length ||
        (options.length === 1 && options[0].type === NodeType.UP_BRANCH));
    const filterOptions = useCallback((options, state) => {
        const { upBranch, freeSoloOptions, branchOptionsMap, leafOptionsMap, optionKeys, } = options.reduce((result, option) => {
            if (option.type === NodeType.UP_BRANCH) {
        ...
        const filteredOptions = (() => {
            const [branchOptions, leafOptions] = filterOptionsProp(Array.from(optionKeys), {
                ...state,
                getOptionLabel: getOptionLabelProp,
            }).reduce((filteredOptions, node) => {
    ...
    const onHighlightChange = useCallback((event, option, reason) => {
        ...
        if (onHighlightChangeProp) {
            onHighlightChangeProp(event, (_a = option === null || option === void 0 ? void 0 : option.node) !== null && _a !== void 0 ? _a : null, reason);
        }
    ...
    const onInputChange = useCallback((...args) => {
        ...
        if (selectedOption.current &&
            ...
            reason === "reset") {
            if (multiple || (value !== null && value !== void 0 ? value : null) === null) {
                args[1] = "";
            }
            else {
    ...
    const onKeyDown = useCallback((event) => {
        ...
        if (!selectedOption.current ||
            ...
            if (selectedOption.current.type === NodeType.UP_BRANCH) {
                ...
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                event["defaultMuiPrevented"] = true;
                const node = (_a = selectedOption.current.path[0]) !== null && _a !== void 0 ? _a : null;
                if (onInputChange) {
                    onInputChange(event, "", "reset");
                }
                if (onBranchChange) {
        ...
        const handleBranchChange = (event, option) => {
            ...
            if (isUpBranch || option.type === NodeType.DOWN_BRANCH) {
                const node = isUpBranch
                    ? (_a = option.path[0]) !== null && _a !== void 0 ? _a : null
                    : option.node;
                if (onBranchChange) {
            ...
            if (multiple) {
                switch (reason) {
                    case "selectOption": {
                        ...
                        if (onChangeProp) {
                            onChangeProp(event, values, reasonIsBlur ? "blur" : reason, details
                                ? {
                                    ...details,
                    ...
                    case "clear": {
                        ...
                        const values = rawValues.map(({ node }) => node);
                        if (onChangeProp) {
                            onChangeProp(event, values, reason, details
                                ? {
                                    ...details,
                ...
                switch (reason) {
                    case "selectOption": {
                        ...
                        if (onChangeProp) {
                            onChangeProp(event, value.node, reasonIsBlur ? "blur" : reason, details
                                ? {
                                    ...details,
                                    option: value.node,
                                }
                    ...
                    case "createOption": {
                        ...
                        const freeSoloNode = new FreeSoloNode(freeSoloValue, curBranch);
                        if (onChangeProp) {
                            onChangeProp(event, freeSoloNode, reasonIsBlur ? "blur" : reason, details
                                ? {
                                    ...details,
                                    option: freeSoloNode,
                                }
                    ...
                    case "clear": {
                        const [, , , details] = args;
                        if (onChangeProp) {
                            onChangeProp(event, null, reasonIsBlur ? "blur" : reason, details
                                ? {
                                    ...details,
                                    option: details.option.node,
                                }
    ...
    if (process.env.NODE_ENV !== "production") {
        const missingValue = (() => {
            ...
            if (value !== null && !freeSolo && _return.options.length > 0) {
                return (multiple ? value : [value]).filter((value2) => !_return.options.some((option) => isOptionEqualToValue(option, value2)));
            }
            else {
                return null;
            }
        ...
        _return.options = useMemo(() => (missingValue === null || missingValue === void 0 ? void 0 : missingValue.length)
            ? [..._return.options, ...missingValue]
            : _return.options, [_return.options, missingValue]);
        const _filterOptions = _return.filterOptions;
        // eslint-disable-next-line react-hooks/rules-of-hooks
        _return.filterOptions = useCallback((options, ...rest) => _filterOptions(options.filter((option) => !(missingValue === null || missingValue === void 0 ? void 0 : missingValue.includes(option))), ...rest), [_filterOptions, missingValue]);
    }
    return _return;
};
